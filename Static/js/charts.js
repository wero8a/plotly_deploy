...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var resultSample = sampleNumber[0];

    // 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    var otu_ids = resultSample.otu_ids;
    var otuidsSliced = otu_ids.slice(0,10).map(idNumber => `OTU ${idNumber}`).reverse();
    console.log(otuidsSliced);

    var otu_labels = resultSample.otu_labels;
    var otuLabelsSliced = otu_labels.slice(0,10).reverse();
    ...
    // 10. Use Plotly o tplot the data with the layout. 
    ...
    var sampleMetaNumber= metadata.filter(sampleObj => sampleObj.id == sample) 

    // 2. Create a variable that holds the first sample in the metadata array.
    var resultMeta = sampleMetaNumber[0];
